...
@Component
@ControllerAdvice
public class CouponAPIExceptionHandler extends ResponseEntityExceptionHandler{
	
	private final Logger log = Logger.getLogger(CouponAPIExceptionHandler.class.getName());
	...
	public final ResponseEntity<CustomErrorMessage> couponApiExceptionHandler(CouponApiException ex){
		log.info("in CouponApiException");
		CustomErrorMessage exceptionResponse= new CustomErrorMessage(ex.getMessage(),ex.getMessage());
		log.debug("customErrorMessage : "+exceptionResponse);
		return new ResponseEntity<CustomErrorMessage>(exceptionResponse,new HttpHeaders(),HttpStatus.INTERNAL_SERVER_ERROR);
	}
